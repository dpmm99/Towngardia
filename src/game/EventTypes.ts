...
export class Hauntymonth extends CityEvent {
    ...
    private shouldContinue(date: Date): boolean {
        //Starts on October 1st; ends midnight November 1.
        return date.getMonth() === 9;
    }

    override shouldStart(city: City, date: Date): boolean {
        return this.checkedStart(this.shouldContinue(date) && !city.events.some(p => p.type === this.type), city, date);
    }
    ...
    override calculateDuration(city: City, date: Date): void {
        //Recalculates because new players might start in the middle of the month and Timeslips cause additional ticks in the same real time frame.
        if (!this.shouldContinue(date)) return;
...
export class TourismReward extends CityEvent {
    ...
    override onLongTick(city: City, date: Date): boolean {
        ...
        return super.onLongTick(city, date);
    }
